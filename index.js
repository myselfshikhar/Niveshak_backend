...
app.use(cors({
  origin: [
    ...
    "https://niveshak-dashboard.vercel.app" // ✅ NO trailing slash!
  ],
  credentials: true,
}));
...
app.delete("/deleteOrder/:orderId", async (req, res) => {
  try {
    const result = await OrdersModel.findOneAndDelete({ orderId: req.params.orderId });
    if (!result) {
...
app.post("/sellOrder", async (req, res) => {
  ...
  try {
    // 1. Check if stock exists in holdings
    ...
    if (holding.qty < qty) {
      return res.status(400).json({
        error: `Insufficient quantity. You have only ${holding.qty} shares.`,
      });
    }

    // 3. Proceed with SELL: reduce qty in holdings
    holding.qty -= qty;
